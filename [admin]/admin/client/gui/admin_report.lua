...
function aReport ( )
	if ( aReportForm == nil ) then
		...
		aReportForm		= guiCreateWindow ( x / 2 - 150, y / 2 - 170, 300, 340, "Contatar Admin", false )
					   guiCreateLabel ( 0.05, 0.11, 0.20, 0.07, "Categoria:", true, aReportForm )
					   guiCreateLabel ( 0.05, 0.19, 0.20, 0.07, "Assunto:", true, aReportForm )
					   guiCreateLabel ( 0.05, 0.34, 0.20, 0.07, "Mensagem:", true, aReportForm )
		aReportLblPlayer = guiCreateLabel ( 0.05, 0.27, 0.20, 0.07, "Jogador:", true, aReportForm )
		aReportBtnPlayer = guiCreateButton ( 0.75, 0.27, 0.20, 0.07, "Selecionar", true, aReportForm )
		aReportCategory	= guiCreateEdit ( 0.30, 0.10, 0.65, 0.07, "Pergunta", true, aReportForm )
...
function aReportSelectPlayer ( )
	if ( aSelectPlayer == nil ) then
		local x, y = guiGetScreenSize ( )
		aSelectPlayer = guiCreateWindow ( x / 2 - 155, y / 2 - 250, 310, 500, "Selecionar Jogador", false)
		...
		guiGridListAddColumn ( playerList, "Nome", 0.85 )
		for _, player in pairs (getElementsByType("player")) do
			guiGridListSetItemText(playerList, guiGridListAddRow(playerList), 1, getPlayerName(player), false, false)
		end
		local btnSelectPlayer = guiCreateButton(0.57, 0.93, 0.33, 0.05, "Selecionar", true, aSelectPlayer)
		...
		local btnClose = guiCreateButton(0.10, 0.93, 0.33, 0.05, "Fechar", true, aSelectPlayer)
